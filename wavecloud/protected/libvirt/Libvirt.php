...
class Libvirt 
{
    ...
    function domain_is_running($domain, $name = false) 
    {
        ...
        if (!$tmp)
            return $this->_set_last_error();
        $ret = ( ($tmp['state'] == 1) || ($tmp['state'] == 3) );
        unset($tmp);
        return $ret;
    }

    function domain_is_running2($domain, $name = false)
    {
        $dom = $this->get_domain_object($domain);
        if (!$dom)
            return false;
        ...
        return $this->domain_state_translate($tmp['state']);
    }
    ...
    function domain_state_translate($state) 
    {
        switch ($state) {
            ...
            case 2:  return 'nostate';
            case 3:  return 'blocked';
            case 4:  return 'paused';
            case 5:  return 'shutdown';
            case 6:  return 'shutoff';
